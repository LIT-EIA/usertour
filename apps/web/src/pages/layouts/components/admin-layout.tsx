          ...
          >
            <Button className="bg-gradient-to-r from-sky-100 to-sky-200 space-x-2 text-foreground">
              <img
                ...
                alt=""
              />
              <span className="min-w-28	">{t('nav.installToPublish')}</span>
            </Button>
          </a>
          {/* <Search /> */}
          <AdminEnvSwitcher />
...
export const AdminLayoutNewContent = (props: AdminLayoutNewContentProps) => {
  ...
  return (
    <div className="py-1.5 pr-1.5 w-full min-w-0 flex-shrink">
      <div
        className={cn(
          'w-full min-w-0 overflow-hidden flex flex-col relative rounded-md border border-border bg-white h-full dark:border-border/60 dark:bg-card/60',
          className,
        )}
      >
        <div className="flex justify-end items-center px-3 py-1 border-b flex-shrink-0 dark:border-border/60">
          <LanguageSwitcher />
        </div>
        <div className="group/sidebar-wrapper flex flex-1 min-h-0 w-full">{children}</div>
      </div>
    </div>
  );
};
