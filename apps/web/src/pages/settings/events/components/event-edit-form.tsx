    ...
    async (formValues: FormValues) => {
      ...
      try {
        ...
        if (!ret.data?.updateEvent?.id) {
          showError(t('settings.events.updateFailure'));
        }
        onClose();
      } catch (error) {
      ...
      <DialogContent className="max-w-5xl">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleOnSubmit)}>
            <DialogHeader>
              <DialogTitle>{t('settings.events.editTitle')}</DialogTitle>
            </DialogHeader>
                              ...
                              <SelectTrigger className="w-full">
                                <span className="text-gray-500">
                                  {selectedAttributeValue
                                    ? ''
                                    : t('settings.events.form.attributesPlaceholder')}
                                </span>
                              </SelectTrigger>
                            </FormControl>
