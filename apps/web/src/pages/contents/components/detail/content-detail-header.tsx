              ...
              {!isSaveing && (
                <div className="px-1 text-sm text-muted-foreground min-w-60 text-right">
                  {content.editedVersionId !== content.publishedVersionId && version && (
                    <>{t('contents.detail.autosaved', { when: formatRelativeTime(new Date(version?.updatedAt)) })}</>
                  )}
                  {content.editedVersionId === content.publishedVersionId &&
                    content.publishedAt && (
                      <>{t('contents.detail.autosaved', { when: formatRelativeTime(new Date(content.publishedAt)) })}</>
                    )}
                </div>
              )}
