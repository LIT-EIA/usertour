  ...
  const handleSubmit = async () => {
    try {
      ...
      if (result) {
        toast({
          variant: 'success',
          title: action === 'delete' ? t('sessionActions.toast.deleteSuccess') : t('sessionActions.toast.endSuccess'),
        });
        onSubmit(true);
        return;
      }
      ...
      toast({
        variant: 'destructive',
        title: action === 'delete' ? t('sessionActions.toast.deleteFailed') : t('sessionActions.toast.endFailed'),
      });
    }
  };
...
const SessionForm = ({ session, open, onOpenChange, onSubmit, type }: SessionFormProps) => {
  ...
  const title = type === 'delete' ? t('sessionActions.delete.title') : t('sessionActions.end.title');
  const description = type === 'delete' ? t('sessionActions.delete.description') : t('sessionActions.end.description');
  const confirmButton = type === 'delete' ? t('sessionActions.delete.confirmButton') : t('sessionActions.end.confirmButton');
